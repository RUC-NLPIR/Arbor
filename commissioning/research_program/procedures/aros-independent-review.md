...
1. Verify `packet_sha256` against the canonical frozen packet bytes excluding the
   `packet_sha256` field before dereferencing or reviewing any packet content.
2. Read the exact TaskBrief, root question, Claim draft, preregistration, candidate
...
3. Read and bind `researcher_model_id`, `researcher_model_family`, every
...
4. Obtain an immutable `independence_evidence_ref` that proves the Reviewer context
...
5. Record the exact `reviewer_model_id` and `reviewer_model_family` before
   reproduction, prove that the Reviewer family differs from
   `researcher_model_family`, and bind both identities and the independence evidence to
   every new Reviewer Run and Eval receipt.
6. Independently rebuild the exact `candidate_commit` in a clean isolated Reviewer
   Run without editing the candidate or using `researcher_worktree_ref` or any
   Researcher-built workspace.
7. Independently rerun every primary evidence path from the exact
...
8. Run the specified evaluations through new Reviewer `Eval.run` calls and compare
   both raw outputs and parsed results with the frozen raw evidence; record every
   match, mismatch, and unavailable comparison.
9. Record exact packet, Claim-draft, candidate, source, raw-evidence, reproduction,
...
10. Develop and test plausible alternative explanations, and attempt at least one
    counterexample that could distinguish the reported mechanism from those
    alternatives.
11. Audit leakage and contamination, warmup and startup effects, data identity and
...
12. Place every material issue in the corresponding report field, mark objections
...
- Independence binding: Set `reviewer_model_id`, `reviewer_model_family`, and
  `independence_evidence_ref` to the exact verified Reviewer identity and fresh-context
  evidence.
...
- If `reviewer_model_family` equals `researcher_model_family`, either family is
  unknown or unverifiable, or `independence_evidence_ref` does not prove fresh empty
  history without a Researcher transcript, do not emit a `ReviewerReport`; call
  `Research.petition` with the exact independence blocker and exit incomplete.
